...
define('DB_USERNAME',		"muster"				); // Database username
define('DB_PASSWORD',		"nRSK0zN9ebkH8sWS"		); // Database password
define('DB_NAME',			"muster_ss"				); // Database name

$table_prefix = '';									// Table prefix(if any)
$db_usertable = 'Member';							// Users or members information table name
$db_usertable_userid = 'UUID';					// UserID field in the users or members table
...
class Integration{
	...
	function chatLogin($userName,$userPass) {
		...
		global $guestsMode;
		   ...
		   $sql = ("SELECT m.*, oas.Created as OasCreated, oat.AccessToken FROM Member m  
                    JOIN OauthSession oas ON oas.MemberID = m.ID 
                    JOIN OauthAccessToken oat ON oat.OauthSessionID = oas.ID 
                    WHERE email = '".sql_real_escape_string($userName)."'  
                    ORDER BY OasCreated DESC 
                    LIMIT 1"); 
		...
		if($row['AccessToken'] == $userPass) { //replace $userPass with $salted_password if you have edited the password validation mechanism
		...
		if($userid && function_exists('mcrypt_encrypt') && defined('ENCRYPT_USERID') && ENCRYPT_USERID == '1') {
			...
			$userid = rawurlencode(base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($key), $userid, MCRYPT_MODE_CBC, md5(md5($key)))));
                ...
                if (($userPass == 'cometchat' || $userPass == "CC^CONTROL_USERID"))
                {
                    $sql = ("SELECT * FROM `".DB_USERTABLE."` WHERE " . DB_USERTABLE_USERID . " = '" . mysqli_real_escape_string($GLOBALS['dbh'], $userName) . "'");
