...
if(isset($_REQUEST['action'])) {
	switch ($_REQUEST['action']) {
		...
		break;
		case 'creategroup':
		$params = array('apikeyvalue' => $apikeyvalue, 'groupname' => $groupname, 'grouptype' => $grouptype, 'grouppassword' => $grouppassword, 'groupid' => $groupid, 'userid' => $userid );
		creategroup($params);
		break;
		case 'addgroupusers':
		if($id != null){
	...
	*/
	...
	if(!empty($params['groupid'])){
		...
		if(sql_num_rows($query) == 0) {
			...
			$query = sql_query('api_creategroup', array('name'=>sanitize_core($name), 'createdby'=>$params['userid'], 'lastactivity'=>getTimeStamp(), 'password'=>sanitize_core($params['grouppassword']), 'type'=>sanitize_core($params['grouptype']), 'guid'=>sanitize_core($params['groupid'])));
			$currentroom = sql_insert_id('cometchat_chatrooms');
