...
define('DB_NAME',			"muster"			); // Database name
...
class Integration{
	...
	function chatLogin($userName,$userPass) {
		...
		if(!empty($row)) { //replace $userPass with $salted_password if you have edited the password validation mechanism
		...
		if (is_numeric($userName) && ($userPass == 'cometchat' || $userPass == "CC^CONTROL_USERID")) {
			$sql = ("SELECT * FROM `".TABLE_PREFIX.DB_USERTABLE."` WHERE ".DB_USERTABLE_USERID." = '".mysqli_real_escape_string($GLOBALS['dbh'],$userName)."'");
