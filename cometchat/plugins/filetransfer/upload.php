...
if (empty($errormessage)) {
	...
	if (!empty($_POST['chatroommode'])) {
		if ((!empty($isImage) && $isImage) || (!empty($isVideo) && $isVideo) || (!empty($isAudio) && $isAudio) ) {
			$response = sendChatroomMessage($_POST['to'],'<div style="display:none;">'.$filetransfer_language[9].'</div><br/><a class="imagemessage mediamessage" pluginname ="filetransfer" filename="'.$unencryptedfilename.'" encfilename="'.$md5filename.'" mediatype="'.$mediaType.'" link="'.$server_url.'plugins/filetransfer/download.php?file='.$md5filename.'&amp;unencryptedfilename='.$unencryptedfilename.'"  href="'.$server_url.'plugins/filetransfer/download.php?file='.$md5filename.'&amp;unencryptedfilename='.$unencryptedfilename.'" imageheight="'.$height.'" imagewidth="'.$width.'">'.$imgtag.'</a>',0);
		} else {
			$response = sendChatroomMessage($_POST['to'],$filetransfer_language[9].' ('.$filename.'). <a href="'.$server_url.'plugins/filetransfer/download.php?file='.$md5filename.'&amp;unencryptedfilename='.$unencryptedfilename.'" target="_blank" mediaType="0">'.$filetransfer_language[6].'</a>',0);
		}
	} else {
		if ((!empty($isImage) && $isImage) || (!empty($isVideo) && $isVideo) || (!empty($isAudio) && $isAudio) ) {
