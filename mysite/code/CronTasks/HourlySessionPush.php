...
class HourlySessionPush extends BuildTask {
 
    protected $title = 'Hourly Session Push';
 
    protected $description = 'Push notifications to attendees if session is happening within an hour. Should run one time per hour.';
 
    protected $enabled = true;
 
    function run($request) {
        $push = new Fcm;
        $todaysGamingSessions = GamingSession::get()->filter(['GameStatus' => 'Active', 'StartDate:PartialMatch' => date('Y-m-d H:')]);
        $results = array();
        foreach($todaysGamingSessions as $todaysGamingSession) {
            ...
            try {
                ...
                $msg = 'Your gaming session "' . $todaysGamingSession->Name . '" will start soon';
